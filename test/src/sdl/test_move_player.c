...
#define _ZERO(X) ((X) > -DOUBLE_PRECISION && (X) < DOUBLE_PRECISION)
...
void test_move_player()
{
	...
	for (i = 0, context.me.angle = 0; context.me.angle < 2 * PI; context.me.angle += PI_2, i++) {
		...
		MU_ASSERT(_ZERO(context.me.coord.x - ctrl_b[i].x),					\
				  "x: "CLR_RED"%f != %f "CLR_BLUE"\n(me: %f/%f at %d) "CLR_MAGENTA"\n(ctrl: %f/%f)", \
				  context.me.coord.x, ctrl_b[i].x,						\
				  context.me.coord.x, context.me.coord.y, context.me.angle, \
				  ctrl_b[i].x, ctrl_b[i].y);

		MU_ASSERT(_ZERO(context.me.coord.y - ctrl_b[i].y),					\
				  "y: "CLR_RED"%f != %f "CLR_BLUE"\n(me: %f/%f at %d) "CLR_MAGENTA"\n(ctrl: %f/%f)", \
	...
	for (i = 0, context.me.angle = PI_4; context.me.angle < 2 * PI; context.me.angle += PI_2, i++) {
		...
		MU_ASSERT(_ZERO(context.me.coord.x - ctrl_c[i].x),				\
				  "x: "CLR_RED"%f != %f "CLR_BLUE"\n(me: %f/%f at %d) "CLR_MAGENTA"\n(ctrl: %f/%f)", \
				  context.me.coord.x, ctrl_c[i].x,						\
				  context.me.coord.x, context.me.coord.y, context.me.angle, \
				  ctrl_c[i].x, ctrl_c[i].y);

		MU_ASSERT(_ZERO(context.me.coord.y - ctrl_c[i].y),					\
				  "y: "CLR_RED"%f != %f "CLR_BLUE"\n(me: %f/%f at %d) "CLR_MAGENTA"\n(ctrl: %f/%f)", \
